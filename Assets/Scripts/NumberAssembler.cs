...
public class NumberAssembler : MonoBehaviour {
    ...
    public float Number {
        ...
        set {
            ...
            ((BoxCollider2D)collider2D).center = new Vector2(((BoxCollider2D)collider2D).center.x + (numberCount - 1) * 0.25f, ((BoxCollider2D)collider2D).center.y);
            ((BoxCollider2D)collider2D).size = new Vector2(((BoxCollider2D)collider2D).size.x + (numberCount - 1) * 0.40f, ((BoxCollider2D)collider2D).size.y);
        }
    }
